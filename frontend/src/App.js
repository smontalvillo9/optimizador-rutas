  ...
  const ProveedorCard = ({ proveedor, onClick, isSelected }) => (
    ...
    >
      ...
      <div className="text-sm text-gray-600 mt-1">
        {proveedor.total_vehiculos || 0} vehículos • {proveedor.capacidad_total || 0} combis
      </div>
    </div>
  );
        ...
        {vehiculosFiltrados.map(vehiculo => (
          <div key={vehiculo.id} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
            ...
            <div className="text-right">
              <div className="text-sm font-medium">{vehiculo.capacidad_combis} combis</div>
      ...
      <div className="bg-white rounded-lg shadow-sm border p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div className="text-center p-3 bg-blue-50 rounded-lg">
            <div className="text-2xl font-bold text-blue-600">{result.metricas.eficiencia_llenado}%</div>
            <div className="text-xs text-gray-600">Eficiencia</div>
          </div>
          <div className="text-center p-3 bg-green-50 rounded-lg">
            <div className="text-2xl font-bold text-green-600">{result.metricas.vehiculos_necesarios}</div>
            <div className="text-xs text-gray-600">Vehículos</div>
          </div>
          <div className="text-center p-3 bg-yellow-50 rounded-lg">
            <div className="text-2xl font-bold text-yellow-600">{result.metricas.tiendas_asignadas}</div>
            <div className="text-xs text-gray-600">Tiendas</div>
          </div>
          <div className="text-center p-3 bg-purple-50 rounded-lg">
            <div className="text-2xl font-bold text-purple-600">{result.metricas.capacidad_usada}</div>
        ...
        <div className="space-y-4">
          ...
          {result.rutas.map((ruta, index) => (
            <div key={index} className="border border-gray-200 rounded-lg p-4">
              <div className="flex justify-between items-center mb-3">
                <h5 className="font-medium">{ruta.vehiculo_nombre}</h5>
                <div className="text-sm text-gray-600">
                  {ruta.eficiencia.toFixed(1)}% llenado
                </div>
              </div>
              
              <div className="mb-2">
                <div className="flex justify-between text-sm mb-1">
                  <span>{ruta.capacidad_usada.toFixed(1)} combis</span>
                  <span>{ruta.capacidad_maxima} combis máx</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div 
                    className="bg-blue-500 h-2 rounded-full transition-all duration-300"
                    style={{ width: `${Math.min(100, ruta.eficiencia)}%` }}
                  ></div>
                </div>
              </div>
              ...
              <div className="text-sm text-gray-600">
                ...
                {ruta.tiendas.slice(0, 3).map(t => t.nombre).join(', ')}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="text-center p-4 bg-yellow-50 rounded-lg">
                    <div className="text-3xl font-bold text-yellow-600">
                      {vehiculos.reduce((sum, v) => sum + (v.capacidad_combis || 0), 0)}
                    </div>
                  ...
                  >
                    ...
                    {proveedores.map(proveedor => (
                      <option key={proveedor.id} value={proveedor.id}>
                        {proveedor.nombre} ({proveedor.total_vehiculos} vehículos)
                      </option>
                    ))}
                  </select>
                </div>
              ...
              {uploadResult && (
                <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
                  ...
                  <ul className="text-sm text-green-700 mt-2">
                    <li>• Vehículos importados: {uploadResult.vehiculos_importados}</li>
                    <li>• Tiendas importadas: {uploadResult.tiendas_importadas}</li>
